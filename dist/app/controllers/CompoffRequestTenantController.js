...
// import CompoffRequestTenant from '@/app/models/CompoffRequestTenant'
// import ProjectTenant from '@/app/models/ProjectTenant'
...
const CompoffRequestTenant_1 = __importDefault(require("../models/CompoffRequestTenant"));
...
class CompoffRequestTenantController extends AbstractController_1.default {
    // private async setInstanceModel(_req: Request) {
    // const header = req.header('tenantId')
    // const tenantId = header ? header : ''
    // this.model = this.model ? this.model : CompoffRequestTenant(tenantId)
    // this.CompoffRequestTenantModel = this.CompoffRequestTenantModel
    //   ? this.CompoffRequestTenantModel
    //   : this.model
    // this.ProjectTenantModel = this.ProjectTenantModel
    //   ? this.ProjectTenantModel
    ...
    isCreator(req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { body: { reId, id = reId, _id = id, tenantId }, user, } = req;
            try {
                const item = yield CompoffRequestTenant_1.default.getInstance(tenantId)
    ...
    tenantId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // get project manager
            const projectManagerDB = yield ProjectTenant_1.default.getInstance(tenantId).findById(projectId);
            ...
            const employeeDB = yield EmployeeTenant_1.default.getInstance(tenantId).findOne({
                _id: employeeId,
            });
            if (!employeeDB) {
                throw new AdvancedError_1.default({
            ...
            const locationId = employeeDB.location;
            const employees = yield EmployeeTenant_1.default.getInstance(tenantId).aggregate([
                {
                    $match: {
                        location: locationId,
                    },
            ...
            const users = yield ManagePermission_1.default.aggregate([
                {
                    $match: {
                        roles: {
                            $in: ['REGION-HEAD'],
            ...
            const adminCLA = yield EmployeeTenant_1.default.getInstance(tenantId).findOne({
                _id: adminCLAId,
            });
    ...
    getApprovalFlow(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { employeeId, projectId, tenantId } = req.body;
            const data = yield this.getApprovalFlowFunc(employeeId, projectId, tenantId);
    ...
    add(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { project, extraTime, description, cc, action, duration, tenantId, } = req.body;
            const { user } = req;
            const existInfo = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
            ...
            const existEmployee = yield EmployeeTenant_1.default.getInstance(tenantId).findOne({
                generalInfo: existInfo._id,
            });
            if (!existEmployee) {
                throw new AdvancedError_1.default({
                    employee: {
            ...
            const { employee = existEmployee._id, location = existEmployee.location, company = existEmployee.company, manager = existEmployee.manager ? existEmployee.manager : '', } = user;
            if (manager === '') {
                throw new AdvancedError_1.default({
                    manager: {
                        path: 'method',
            ...
            const approvalFlow = yield this.getApprovalFlowFunc(existEmployee._id, project, tenantId);
            if (!approvalFlow) {
                throw new AdvancedError_1.default({
                    leaverequest: {
                        kind: 'not.found',
            ...
            // )
            let hrManagers = yield this.getEmployeeByRole('HR-MANAGER', existEmployee.location, req);
            const ccId = cc.map((person) => mongoose_1.Types.ObjectId(person));
            let ccEmployees;
            let ccEmails = [];
            if (cc.length > 0) {
                ccEmployees = yield EmployeeTenant_1.default.getInstance(tenantId).aggregate([
                    {
                        $match: {
                            _id: { $in: ccId },
                        },
            ...
            const requesteeName = `${existInfo.firstName} ${existInfo.lastName}`;
        ...
        <ul>${times}</ul>`;
            ...
            if (employee) {
                const empExist = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                    employee: employee,
                });
                const employeeName = `${empExist.firstName} ${empExist.lastName}`;
                const employeeEmail = empExist.workEmail;
            ...
            const projectInfo = yield ProjectTenant_1.default.getInstance(tenantId).findById(project);
            if (projectInfo.manager) {
                const managerInfo = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                    employee: projectInfo.manager,
                });
                const managerName = `${managerInfo.firstName} ${managerInfo.lastName}`;
                const managerEmail = managerInfo.workEmail;
                yield SendMail_1.sendNotificationCompoff({
                    email: managerEmail,
            ...
            let newItem = yield CompoffRequestTenant_1.default.getInstance(tenantId).create(data);
            let item = yield newItem.save();
            res.send(new ResponseResult_1.default({
                data: item,
    ...
    update(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { user } = req;
            const existEmployee = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                workEmail: user.email,
            });
            const employeeLocation = yield EmployeeTenant_1.default.getInstance(tenantId).findById(existEmployee.employee);
            const data = yield CompoffRequestTenant_1.default.getInstance(tenantId).findById(_id);
            // if (!checkUser) {
            ...
            data.set(req.body);
            yield data.save();
            let hrManagers = yield this.getEmployeeByRole('HR-MANAGER', employeeLocation.location, req);
            const { cc } = data;
            const ccId = cc.map((person) => mongoose_1.Types.ObjectId(person));
            let ccEmployees;
            let ccEmails = [];
            ...
            const requesteeName = `${existEmployee.firstName} ${existEmployee.lastName}`;
      ...
      `;
            // Mail notify to employee
            if (existEmployee) {
                const employeeName = `${existEmployee.firstName} ${existEmployee.lastName}`;
                const employeeEmail = existEmployee.workEmail;
                yield SendMail_1.sendNotificationCompoff({
                    email: employeeEmail,
                    fullName: employeeName,
                }, [], mailTitle, mailContent);
            ...
            // Mail notify to Project manager
            const projectInfo = yield ProjectTenant_1.default.getInstance(tenantId).findById(data.project._id);
            if (projectInfo.manager) {
                const managerInfo = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                    employee: projectInfo.manager,
                });
    ...
    remove(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const existEmployee = yield EmployeeTenant_1.default.getInstance(tenantId).findById(existInfo.employee);
            const item = yield CompoffRequestTenant_1.default.getInstance(tenantId)
                .findById(id)
                .exec();
            if (!item) {
            ...
            yield HistoryService_1.default.put({
                ...
                company: existEmployee.company,
                location: existEmployee.location,
            });
            let hrManagers = yield this.getEmployeeByRole('HR-MANAGER', existEmployee.location, req);
            const { cc } = item;
            const ccId = cc.map((person) => mongoose_1.Types.ObjectId(person));
            ...
            // Mail notify to employee
            if (existEmployee) {
                const employeeName = `${existInfo.firstName} ${existInfo.lastName}`;
                const employeeEmail = existInfo.workEmail;
                console.log('903', {
                    email: employeeEmail,
                    fullName: employeeName,
                }, [], mailTitle, mailContent);
            ...
            // Mail notify to Project manager
            const projectInfo = yield ProjectTenant_1.default.getInstance(tenantId).findById(item.project._id);
            if (projectInfo.manager) {
                const managerInfo = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                    employee: projectInfo.manager,
                });
    ...
    getByEmployee(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const existRequest = yield CompoffRequestTenant_1.default.getInstance(tenantId).findById(employee);
            if (!existRequest) {
                throw new AdvancedError_1.default({
                    compoffRequest: {
                        kind: 'not.found',
    ...
    getByID(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const items = (yield CompoffRequestTenant_1.default.getInstance(tenantId).aggregate(aggregate));
            if (!items.length) {
                throw new AdvancedError_1.default({
                    compoffrequest: {
                        kind: 'not.found',
    ...
    getTeamRequest(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            lodash_1.map(req.user.signInRole, (per) => per._id).forEach(item => {
            ...
            if (!checkRoleHR) {
                ...
                matchThree.$match = {
                    ...
                    company: mongoose_1.Types.ObjectId(employee.company),
                };
            ...
            const total = yield CompoffRequestTenant_1.default.getInstance(tenantId).aggregate(aggregate);
            res.send(new ResponseResult_1.default({
                data: {
                    items,
                    total,
    ...
    getUserRequest(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let employee = yield EmployeeTenant_1.default.getInstance(tenantId).findOne({
                generalInfo: generalInfo._id,
            });
            if (!employee) {
                throw new AdvancedError_1.default({
    ...
    approveATicketFunc(_id, req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { tenantId } = req.body;
            const existRequest = yield CompoffRequestTenant_1.default.getInstance(tenantId).findById(_id);
            const employee = yield EmployeeTenant_1.default.getInstance(tenantId).findById(existRequest.employee);
            const currentUser = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                workEmail: req.user.email,
            });
            // const employeeCurrentUser = await EmployeeTenant.getInstance(
            //   tenantId,
            // ).findById(currentUser.employee)
            if (!existRequest) {
                throw new AdvancedError_1.default({
                    compoffrequest: {
                        kind: 'not.found',
            ...
            // check role
            let checkRoleHR = false;
            lodash_1.map(req.user.signInRoles, (per) => per._id).forEach(item => {
                if (lodash_1.includes(['HR', 'HR-MANAGER'], item)) {
                    checkRoleHR = true;
                    return true;
                }
            });
            if (String(currentUser.employee) === String(approvalFlow.step2.employee) ||
                String(currentUser.employee) === String(approvalFlow.step3.employee) ||
                checkRoleHR) {
            ...
            let admins = yield this.getEmployeeByRole('REGION-HEAD', employee.location, req);
            let hrManagers = yield this.getEmployeeByRole('HR-MANAGER', employee.location, req);
            const ccId = cc.map((person) => mongoose_1.Types.ObjectId(person));
            let ccEmployees;
            let ccEmails = [];
            if (cc.length > 0) {
            ...
            const requesteeName = `${currentUser.firstName} ${currentUser.lastName}`;
            ...
            if (employee) {
                ...
                const employeeEmail = currentUser.workEmail;
                yield SendMail_1.sendNotificationCompoff({
                    email: employeeEmail,
                    fullName: employeeName,
                }, [], mailTitle, mailContent);
            ...
            // Mail notify to Project manager
            const projectInfo = yield ProjectTenant_1.default.getInstance(tenantId).findById(existRequest.project._id);
            if (projectInfo.manager) {
                const managerInfo = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                    employee: projectInfo.manager,
                });
    ...
    rejectATicketFunc(_id, comment, req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const existRequest = yield CompoffRequestTenant_1.default.getInstance(tenantId).findById(_id);
            const employee = yield EmployeeTenant_1.default.getInstance(tenantId).findById(existRequest.employee);
            const employeeInfo = yield GeneralInfoTenant_1.default.getInstance(tenantId).findOne({
                employee: employee._id,
            });
            // const employeeCurrentUser = currentUser.employee as IEmployee
            if (!existRequest) {
                throw new AdvancedError_1.default({
                    compoffrequest: {
                        kind: 'not.found',
            ...
            // check role
            let checkRoleHR = false;
            lodash_1.map(currentUser.roles, (per) => per._id).forEach(item => {
                if (lodash_1.includes(['HR', 'HR-MANAGER'], item)) {
                    checkRoleHR = true;
                    return true;
                }
            ...
            const requesteeName = `${employeeInfo.firstName} ${employeeInfo.lastName}`;
    ...
    <p>Comment: ${comment}</p>`;
            ...
            if (employee) {
                ...
                const employeeEmail = employeeInfo.workEmail;
                yield SendMail_1.sendNotificationCompoff({
                    email: employeeEmail,
                    fullName: employeeName,
                }, [], mailTitle, mailContent);
    ...
    updateDraft(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const existRequest = yield CompoffRequestTenant_1.default.getInstance(tenantId).findById(id);
            if (!existRequest) {
                throw new AdvancedError_1.default({
                    compoffrequest: {
                        kind: 'not.found',
    ...
    getEmployeeByRole(role, locationId, req) {
        return __awaiter(this, void 0, void 0, function* () {
            const { tenantId } = req.body;
            let hrManagers;
            let admins;
            ...
            if (role === 'HR-MANAGER') {
                hrManagers = yield ManagePermission_1.default.aggregate([
                    {
                        $lookup: {
                            from: `${tenantId}_employees`,
                            localField: 'employee',
                        ...
                        $match: {
                            $and: [
                                ...
                                },
                                {
                                    ...
                                    location: mongoose_1.Types.ObjectId(locationId),
                                },
                            ],
                        },
                    },
            ...
            else if (role === 'REGION-HEAD') {
                admins = yield ManagePermission_1.default.aggregate([
                    {
                        $lookup: {
                            from: `${tenantId}_employees`,
                            localField: 'employee',
...
exports.default = new CompoffRequestTenantController();
//# sourceMappingURL=CompoffRequestTenantController.js.map